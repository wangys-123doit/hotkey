...
### 应用程序控制增强功能
**更新** 新增了更完善的 ToggleWindow 族函数和路径前缀互换机制，增强了应用程序控制的灵活性和可靠性。

- ToggleWindow 基础函数：提供标准的窗口切换功能，支持已存在窗口的最小化/激活和不存在窗口的启动。
- ToggleWindowByTitle：基于窗口标题的精确匹配，适用于多实例应用的精准控制。
- ToggleWindow2/12：支持额外的窗口类过滤参数，实现更精细的窗口匹配。
- RunAppPathWithPrefixFallback：智能路径解析，支持 C/D 盘前缀互换和协议应用启动。
- GetMainWindowByExe：通过进程名和标题获取主窗口句柄，支持复杂的窗口定位需求。
- GetProcessName：通过 Win32 API 获取进程路径，为应用程序管理提供底层支持。

```mermaid
flowchart TD
A["ToggleWindow 调用"] --> B{"窗口是否存在?"}
B --> |是| C{"窗口是否激活?"}
C --> |是| D["WinMinimize 最小化"]
C --> |否| E["WinActivate 激活"]
B --> |否| F["RunAppPathWithPrefixFallback 启动"]
F --> G{"协议应用?"}
G --> |是| H["直接运行协议路径"]
G --> |否| I{"主路径存在?"}
I --> |是| J["运行主路径"]
...
**图示来源**
- [hotkey.ahk:142-165](file://hotkey.ahk#L142-L165)
- [hotkey.ahk:85-127](file://hotkey.ahk#L85-L127)
- [hotkey.ahk:258-269](file://hotkey.ahk#L258-L269)

**章节来源**
- [hotkey.ahk:142-165](file://hotkey.ahk#L142-L165)
- [hotkey.ahk:85-127](file://hotkey.ahk#L85-L127)
- [hotkey.ahk:241-269](file://hotkey.ahk#L241-L269)
...
- 应用程序控制：ToggleWindow 函数采用快速路径检查，减少不必要的窗口枚举；路径前缀互换支持协议应用的即时启动。
...
- 应用程序控制：若 ToggleWindow 失败，检查路径解析是否正确；确认协议应用格式是否符合要求。
